document.addEventListener('DOMContentLoaded', () => {
  ...
  function showPage(pageId: string) {
    ...
    navLinks.forEach(link => {
      link.classList.toggle('active', (link as HTMLElement).dataset.page === pageId);
    });
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      const pageId = (e.target as HTMLElement).dataset.page;
      if (pageId) {
        showPage(pageId);
      }
    });
  ...
  function updateSlider() {
      if (!slider) return;
      (slider as HTMLElement).style.transform = `translateX(-${currentIndex * 100}%)`;
  }
  ...
  function validateField(id: string, requiredMsg: string, validationFn: (value: string) => boolean = (value) => value.trim() !== ''): boolean {
  ...
  const prevLightboxBtn = document.querySelector('.lightbox-prev') as HTMLElement | null;
  const nextLightboxBtn = document.querySelector('.lightbox-next') as HTMLElement | null;

  let currentImageIndex = 0;
  const imagesArray = Array.from(galleryItems) as HTMLImageElement[];
